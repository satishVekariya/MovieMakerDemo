...
open class VideoCompositor: NSObject, AVFoundation.AVVideoCompositing  {
    ...
    open func newRenderedPixelBufferForRequest(request: AVAsynchronousVideoCompositionRequest) -> CVPixelBuffer? {
        ...
        // Background
        let imageWithColor = CIImage(color: CIColor(red: 0, green: 0, blue: 0))
        let backgroundImage = imageWithColor.cropped(to: image.extent)
        image = backgroundImage.composited(over: image)
        
        if let destiImage = instruction.apply(request: request) {
            image = destiImage.composited(over: image)
        }
